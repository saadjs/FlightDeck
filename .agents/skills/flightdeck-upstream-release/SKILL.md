...
3. Before editing, report the proposed upstream tag, build/cask/tag mapping, expected conflict areas, test plan, and canonical cask impact.
...
- Set `--build-version` to the numeric AeroSpace release version: `v0.21.3-Beta` becomes `0.21.3`. This supplies the macOS app's numeric `CFBundleShortVersionString`, so do not pass `-Beta` to `--build-version`.
- Set `--cask-version` to the upstream release version without its `v` prefix: `v0.21.3-Beta` becomes `0.21.3-Beta`. When `--upstream-tag` is supplied, the release wrapper derives and validates this automatically.
- Use a distinct FlightDeck release tag that preserves the upstream version, such as `flightdeck-v0.21.3-Beta`; never reuse the literal upstream tag because Git tags share a namespace.
- Publish every released upstream version through the canonical `flightdeck` cask. Do not recreate `flightdeck-beta`; the tap maps that retired token to `flightdeck` in `cask_renames.json`.
...
./build-release.sh --build-version X.Y.Z --cask-version X.Y.Z-Beta --skip-notarization
```
...
It runs tests, notarizes and staples the build, packages the ZIP, and writes the generated canonical cask to `$HOME/src/homebrew-tap/Casks/flightdeck.rb` (or `FLIGHTDECK_HOMEBREW_TAP_PATH`). Do not tag or publish if it fails.
...
5. Commit and push `Casks/flightdeck.rb` in `saadjs/homebrew-tap`. For the one-time channel consolidation, also commit the deletion of `Casks/flightdeck-beta.rb` and the `cask_renames.json` mapping from `flightdeck-beta` to `flightdeck`.

Report the upstream tag, FlightDeck release URL, release commit, notarization status, cask/tap commit, test results, and any cask-token migration instruction.
