...
Pre-release builds are published through a separate `flightdeck-beta` cask:

```bash
brew install --cask saadjs/tap/flightdeck-beta
```

Beta builds are signed and notarized like stable releases.
When matching an AeroSpace beta, FlightDeck keeps the macOS app version numeric (for example, `0.21.3`) and
identifies the prerelease through its GitHub release tag and this beta cask.
The beta cask conflicts with the stable `flightdeck` cask (they install the same `FlightDeck.app`), so uninstall one before installing the other:

```bash
brew uninstall --cask flightdeck       # when switching to beta
brew uninstall --cask flightdeck-beta  # when switching back to stable
```

Your config and state are untouched when switching casks.
...
1. `~/.flightdeck.toml`
1. `${XDG_CONFIG_HOME}/flightdeck/flightdeck.toml`
1. `~/.aerospace.toml`
