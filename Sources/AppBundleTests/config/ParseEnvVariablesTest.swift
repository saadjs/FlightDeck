...
final class ParseEnvVariablesTest: XCTestCase {
    ...
    func testCyclicDep() {
        ...
        assertEquals(errors, [
            "[ERROR] exec.env-vars.BAR: Env variable 'FOO' isn't presented in FlightDeck.app env vars, or not available for interpolation (because it's mutated)",
            "[ERROR] exec.env-vars.FOO: Env variable 'BAR' isn't presented in FlightDeck.app env vars, or not available for interpolation (because it's mutated)",
        ])
    }
